...
{
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, menuItemWidth , menuScrollViewHeight - indicatorHeight + 4 )];
...
- (void)initValues
{
    ...
    _menuHeight                           = 60.0; //item高度
    _menuMargin                           = 15.0; //每个item的间隙
    _menuItemWidth                        = 111.0;//item宽度
    _selectionIndicatorHeight             = 4.0;  //标示条的高度
    _totalMenuItemWidthIfDifferentWidths  = 0.0;
    //滑动动画的时间 movetopage时有用 手势切换时没用 ms
    ...
    _selectedMenuItemLabelColor   = [UIColor colorWithRed:13/255.f green:88/255.f blue:55/255.f alpha:1]; //已选字体颜色
    _unselectedMenuItemLabelColor = [UIColor colorWithRed:155/255.f green:155/255.f blue:155/255.f alpha:1];//未选字体颜色
    _scrollMenuBackgroundColor    = [UIColor blackColor];//menu(滚动条)颜色
    _viewBackgroundColor          = [UIColor whiteColor];//背景色
    _bottomMenuHairlineColor      = [UIColor clearColor];//menu下线的颜色
    _menuItemSeparatorColor       = [UIColor redColor];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if ([scrollView isEqual:_controllerScrollView]) {
        ...
        int i = _controllerScrollView.contentOffset.x / WIDTH;
#warning mark - 改页面要改这个。
