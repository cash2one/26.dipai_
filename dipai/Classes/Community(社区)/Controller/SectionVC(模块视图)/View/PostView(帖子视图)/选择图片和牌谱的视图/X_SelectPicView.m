...
//    }
    ...
    if (dataSource.count < 10) { // 显示添加图片的按钮
        _isShownPlus = YES;
        NSLog(@"%@", arr);
...
//        if (self.dataArr.count == [collectionView numberOfItemsInSection:0]) {
...
//        }
    ...
    if (_isShownPlus) {
        ...
        if (self.dataArr.count > 9 && indexPath.row == self.dataArr.count-1) {
            cell.btn.hidden = YES;
            cell.imgV.hidden = YES;
            cell.imgV0.hidden = YES;
        }
